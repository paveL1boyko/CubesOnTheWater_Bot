...
class Tapper:
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            web_view = await self.tg_client.invoke(
                RequestWebView(
                    peer=peer, bot=peer, platform="android", from_bot_menu=False, url="https://www.thecubes.xyz/"
                )
            )
            ...
            return parse_qs(web_view.url.split("#")[-1])["tgWebAppData"][0]
    ...
    async def login(self, http_client: aiohttp.ClientSession, init_data: str):
        try:
            async with http_client.post(
                url="https://server.cubeslabs.xyz/auth", json={"initData": init_data, "ref": None}
            ) as response:
    ...
    async def get_tg_x(self, http_client: aiohttp.ClientSession, token: str):
        try:
            async with http_client.post(
                url="https://server.cubeslabs.xyz/auth/trustmebro", json={"token": token, "type": "telegram"}
            ) as response:
                response_text = await response.text()
                if response.status == 200:
                    async with http_client.post(
                        url="https://server.cubeslabs.xyz/auth/trustmebro", json={"token": token, "type": "twitter"}
                    ) as response1:
    ...
    async def join_to_pool(self, http_client: aiohttp.ClientSession, token: str):
        try:
            async with http_client.post(
                url="https://server.cubeslabs.xyz/pools/542/join", json={"token": token}
            ) as response:
                response_text = await response.text()
        except Exception as error:
    ...
    async def mine(self, http_client: aiohttp.ClientSession, token: str):
        try:
            async with http_client.post(
                url="https://server.cubeslabs.xyz/game/mined", json={"token": token}
            ) as response:
                response_text = await response.text()
                if response.status == 200:
                ...
                if response_text == "???????????????":
                    return None
                if response_text == "? banned ?":
                    return "energy recovery"
                if response_text == "Not enough energy":
                    return "not enough"
    ...
    async def boost_pool(self, http_client: aiohttp.ClientSession, token: str, amount: int):
        try:
            async with http_client.post(
                url="https://server.cubeslabs.xyz/pools/boost", json={"amount": amount, "token": token}
            ) as response:
                response_text = await response.text()
                if response.status == 200:
    ...
    async def claim_boxes(self, http_client: aiohttp.ClientSession, token: str):
        try:
            async with http_client.post(
                url="https://server.cubeslabs.xyz/pools/claim", json={"token": token}
            ) as response:
                response_text = await response.text()
    ...
    async def run(self, proxy: str | None) -> None:
        while True:
            ...
            app_user_data = await self.login(http_client=http_client, init_data=tg_web_data)
            ...
            if app_user_data is not None:
                ...
                if app_user_data.get("banned_until_restore") == "true":
                    ...
                    boost_json = await self.boost_pool(
                    ...
                    if boost_json:
                        logger.success(
                            f"{self.session_name} | Boosted pool for better rewards | invested: "
                ...
                time_before_claim = randint(
                    a=settings.TIME_BETWEEN_RECEIVING_BOXES[0], b=settings.TIME_BETWEEN_RECEIVING_BOXES[1]
                )
                while True:
                    try:
                        ...
                        if mine_data == "not enough":
                            logger.warning(
                                f"{self.session_name} | Not enough energy to mine block | " f"Going sleep 15 sec"
                            )
                        ...
                        if mine_data is None:
                            ...
                            if len(mine_data.get("mystery_ids")) > 0 and int(mine_data.get("mystery_ids")[0]) == int(
                                mine_data.get("mined_count")
                            ):
                                logger.info(
                                    f"{self.session_name} | Mined <magenta>mystery box</magenta>! | Drops: "
                                    f"{mine_data.get('drops_amount')}; Energy: {mine_data.get('energy')}; "
                                    f"Boxes: {mine_data.get('boxes_amount')}"
                                )
                            else:
                                logger.info(
                                    f"{self.session_name} | Mined! | Drops: "
                        ...
                        if (time() - last_claim_time) >= time_before_claim and (
                            mine_data is not None or mine_data != {}
                        ):
                            boxes_before_claim = int(mine_data.get("boxes_amount"))
                            boxes_after_claim = await self.claim_boxes(
                                http_client=http_client, token=app_user_data.get("token")
                            )
                            if boxes_after_claim:
                                logger.info(
                                    f"{self.session_name} | Received "
